...
expressApp.cache = cache;
expressApp.config = {
  'USERDB' : process.env.USERDB || 'octopus_users',
  'DOCSDB' : process.env.DOCSDB || 'octopus_docs',
}
...
(async () => {
  for await (let {socket} of agServer.listener('connection')) {
    // Handle socket connection.    // Handle socket connection.
    ...
    (async () => {
      for await (let request of socket.procedure('login')){
        ...
        try {
          ...
          const decodedToken = await auth().verifyIdToken(request.data);
          ...
          socket.setAuthToken({uid: decodedToken.uid});
          log('LOGIN', decodedToken.uid);

          // End the 'login' request successfully.
          request.end();
        } catch (e) {
          console.error(cc.red(` [LOGIN] ${e}`));
